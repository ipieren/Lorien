...
static func save_project(project: Project) -> void:
	...
	# Open file
	var file = FileAccess.open(project.filepath, FileAccess.WRITE)
	var err = FileAccess.open_compressed(project.filepath, FileAccess.WRITE, COMPRESSION_METHOD)
	if err != OK:
...
static func load_project(project: Project) -> void:
	...
	# Open file
	var file = FileAccess.open(project.filepath, FileAccess.READ)
	var file_comp = FileAccess.open_compressed(project.filepath, FileAccess.READ, COMPRESSION_METHOD)
	if file_comp.get_error() != OK:
