...
func end_stroke() -> void:
	if _current_stroke != null:
		...
		if points.size() <= 1 || (points.size() == 2 && points.front().is_equal_approx(points.back())):
			...
			_current_project.undo_redo.add_undo_method(Callable(self, "undo_last_stroke"))
			_current_project.undo_redo.add_undo_reference(_current_stroke)
			# TODO: check if the callable works here
			_current_project.undo_redo.add_do_method(Callable(_strokes_parent, "add_child"))
			...
			_current_project.undo_redo.add_do_method(Callable(_current_project, "add_stroke"))
...
func _delete_selected_strokes() -> void:
	...
	if !strokes.is_empty():
		...
		for stroke in strokes:
			...
			_current_project.undo_redo.add_do_method(Callable(self, "_do_delete_stroke"))
			_current_project.undo_redo.add_undo_reference(stroke)
			_current_project.undo_redo.add_undo_method(Callable(self, "_undo_delete_stroke"))
		_selection_tool.deselect_all_strokes()
		_current_project.undo_redo.commit_action()
		_current_project.dirty = true
